...
FL_list = "FL000,FL200,FL250,FL550"
FL_list = FL_list.split(",")
...
if a.shape[0] == 0 :
    ...
    for j in range(n_levels-1):
        ...
        print "FL : ",FL_list[j]," ",FL_list[j+1]

        loading3D_sum_kg_level = np.zeros((lat_unique.shape[0],lon_unique.shape[0]))

        column = 0

        for i in range(npart):
 
            index = column + init_col
            ...
            if np.sum(conc) == 0:
                ...
                loading_i = loading3D[:,:,index]
                loading3D_sum_kg_level += loading_i * pixel_area * (int(H_LEVELS[j+1,0])-int(H_LEVELS[j,0]))
        

        init_col = init_col + 1

        print "Ash mass loading at flight level ",FL_list[j]," ",FL_list[j+1]," : ",total_mass," kg"

        f = plt.figure(i+1)
        plt.rcParams["font.size"] = 8.0

        loading3D_sum = (loading3D_sum_kg_level * 1000) / (pixel_area * (int(H_LEVELS[j+1,0])-int(H_LEVELS[j,0])))
        ...
        cmap = mpl.colors.ListedColormap(['gray','gray' ])
        cmap.set_over('red')
        cmap.set_under('cyan')
        ...
        #clb.set_label('Loading (g/m^3)', labelpad=-40, y=1.05, rotation=0)

        f.savefig(runname+'_'+day+'_'+time+'_FL_'+str(FL_list[j])+'_'+str(FL_list[j+1])+'.pdf', bbox_inches='tight')
        plt.close()
        #plt.show()   
