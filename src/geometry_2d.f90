...
MODULE geometry_2d
  ...
  INTEGER :: j_source , k_source
  INTEGER :: j_source_W , k_source_S

  REAL(wp), ALLOCATABLE :: cell_fract(:,:)
  REAL(wp), ALLOCATABLE :: dx_rel(:,:)
  REAL(wp), ALLOCATABLE :: dy_rel(:,:)

  REAL(wp), ALLOCATABLE :: upwind_dist(:,:)
  REAL(wp), ALLOCATABLE :: crosswind_dist(:,:)
...
CONTAINS
  ...
  SUBROUTINE init_grid
    ...
    ALLOCATE( dx_rel(comp_cells_x,comp_cells_y) )
    ALLOCATE( dy_rel(comp_cells_x,comp_cells_y) )

    ALLOCATE( upwind_dist(comp_cells_x,comp_cells_y) )
    ALLOCATE( crosswind_dist(comp_cells_x,comp_cells_y) )
  ...
  SUBROUTINE deallocate_grid
    ...
    DEALLOCATE( dist_sourceE )
    DEALLOCATE( dist_sourceW )
    DEALLOCATE( dist_sourceN )
    DEALLOCATE( dist_sourceS )

    DEALLOCATE( sourceE_vect_x )
    DEALLOCATE( sourceE_vect_y )
    DEALLOCATE( sourceW_vect_x )
    DEALLOCATE( sourceW_vect_y )
    DEALLOCATE( sourceS_vect_x )
    DEALLOCATE( sourceS_vect_y )
    DEALLOCATE( sourceN_vect_x )
    DEALLOCATE( sourceN_vect_y )

    DEALLOCATE( grid_output )

    DEALLOCATE( cell_fract )
    DEALLOCATE( dx_rel )
    DEALLOCATE( dy_rel )

    DEALLOCATE( upwind_dist )
    DEALLOCATE( crosswind_dist )
  ...
  SUBROUTINE init_source
    ...
    j_source = FLOOR( ( x_source - x0 ) / cell_size )
    k_source = FLOOR( ( y_source - y0 ) / cell_size )

    j_source_W = FLOOR( ( x_source - r_source - x0 ) / cell_size ) 
    k_source_S = FLOOR( ( y_source - r_source - y0 ) / cell_size ) 
  ...
  SUBROUTINE compute_cell_fract
    ...
    USE parameters_2d, ONLY: x_source, y_source, r_source , dr_dz
    ...
    DO j=1,comp_cells_x
      ...
      DO k=1,comp_cells_y
         ...
         IF ( ( x_stag(j+1) .LT. ( x_source - r_source ) ) .OR.                             &
              ( x_stag(j) .GT. ( x_source + r_source ) ) .OR.                               &
              ( y_stag(k+1) .LT. ( y_source - r_source ) ) .OR.                             &
              ( y_stag(k) .GT. ( y_source + r_source ) ) ) THEN
         ...
         ELSE
            ...
            delta_x = x_comp(j) - x_source
            delta_y = y_comp(k) - y_source
            r_rel = MIN( 1.0_wp , SQRT( delta_x**2 + delta_y**2 ) / r_source )

            dx_rel(j,k) = delta_x / SQRT( delta_x**2 + delta_y**2 ) * r_rel * dr_dz
            dy_rel(j,k) = delta_y / SQRT( delta_x**2 + delta_y**2 ) * r_rel * dr_dz
